...
function showPostLists(event) {
    ...
    for (let post of filteredPosts) {
        if (Number(post.id) > page + 3) continue
        const newPostList = appPostList.cloneNode(true);
    ...
    for (let i = 1; i < Math.floor(posts.length / 3); i++) {
        let navButton = document.createElement("span")
        navButton.className = "pagination-nav"
...
function createPost() {
    ...
    posts.push(post);
    window.alert(`Your post is about ${post.title} with content of ${post.body}, is added`)
...
function postEdit(event) {
    const post = event.target.closest(".app__post-list");
    const postId = post.querySelector(".edit-post").textContent;
    const postBody = post.querySelector(".app__post-content").textContent;
    const title = prompt("Enter new title", post.querySelector(".app__post-content").textContent);
    const body = prompt("Enter new body", postBody);
